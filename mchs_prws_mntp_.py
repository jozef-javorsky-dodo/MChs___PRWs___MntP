...
class MarkovChainSimulator:
    ...
    def monte_carlo_estimate(
        ...
        states = self._rng.integers(
            0, n_states, size=n_samples, dtype=np.int64
        )
    ...
    def plot_distribution_snapshots(
        ...
        try:
            ...
            raise VisualizationError(
                "Failed to generate distribution snapshots"
            ) from err
        finally:
            if fig:
                plt.close(fig)
...
class CityMigrationSimulation:
    ...
    def run(self) -> None:
        try:
            ...
            raise SimulationError(
                "Failed to run city migration simulation"
            ) from exc

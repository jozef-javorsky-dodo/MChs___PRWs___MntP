...
class MarkovChainSimulator:
    ...
    def simulate_random_walks(self) -> NDArrayF64:
        steps = self._rng.choice([-1, 1], size=(self.config.num_walks, self.config.num_steps))
    ...
    def analyze_steady_state(self, transition_matrix: TransitionMatrix,
                             initial_state: NDArrayF64) -> dict[Method, NDArrayF64]:
        ...
        with ThreadPoolExecutor(max_workers=len(self.config.methods)) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.exception("Steady state computation failed with method %s", method)
                    raise SimulationError(f"Failed to compute steady state using {method}") from err
        return results

    def monte_carlo_estimate(self, transition_matrix: TransitionMatrix,
                             initial_state: int | None = None) -> NDArrayF64:
        mat = transition_matrix.matrix
        n = mat.shape[0]
        traj = self._rng.integers(0, n, size=self.config.iterations + 1, dtype=np.int64)
...
class MarkovChainVisualizer:
    ...
    def plot_walks(self, walks: NDArrayF64, show: bool = True,
        ...
        try:
            ...
            for w in walks:
                ax.plot(t, w, alpha=self.config.alpha, linewidth=self.config.linewidth)
            ...
            if save_path:
                ...
                logger.info("Saved plot to %s", fp)
            if show:
                plt.show()
            plt.close(fig)
        except Exception as err:
    ...
    def plot_distribution_snapshots(self, walks: NDArrayF64,
                                    times: list[int] | None = None,
                                    show: bool = True) -> None:
        fig = None
        try:
            pts = times or list(range(20, 101, 20))
            fig, axes = plt.subplots(1, len(pts), figsize=self.config.subplot_figsize,
                                     sharex=True)
            x_vals = np.linspace(self.config.x_range[0], self.config.x_range[1],
                                 self.config.x_points)
            for i, t_val in enumerate(pts):
                axes[i].hist(walks[:, t_val],
                             bins=self.config.hist_bins, density=True,
                             alpha=self.config.hist_alpha, color="skyblue")
...
class CityMigrationSimulation:
    def __init__(self, transition_matrix: TransitionMatrix, population: NDArrayF64,
                 cities: list[str], simulator: MarkovChainSimulator) -> None:
    ...
    def run(self) -> None:
        try:
            results = self.simulator.analyze_steady_state(self.transition_matrix, self.population)
...
class SimulationRunner:
    def __init__(self, simulation_config: SimulationConfig,
                 ...
                 transition_matrix_data: NDArrayF64, population: NDArrayF64,
...
def main() -> None:
    ...
    try:
        ...
        logger.error("Main function failed: %s", err)
    finally:
        plt.close("all")
